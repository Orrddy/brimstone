...
class UpdateController extends Controller
{
    ...
    public function initCleanup(Request $request)
    {
        ...
        try {
            ...
            \Log::error('Update Cleanup Failed: ' . $e->getMessage() . ' : ' . $e->getFile() . ' : ' . $e->getLine());
    ...
    public function download(Request $request)
    {
        ...
        try {
            ...
            \Log::error('Update Download Failed: ' . $e->getMessage() . ' : ' . $e->getFile() . ' : ' . $e->getLine());
            return response()->json(['status' => 'error', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function extract(Request $request)
    {
        ...
        try {
            ...
            \Log::error('Update Extraction Failed: ' . $e->getMessage() . ' : ' . $e->getFile() . ' : ' . $e->getLine());
            return response()->json(['status' => 'error', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function sanitize(Request $request)
    {
        ...
        $files_to_delete = [
            ...
            '/public/assets/json/modules.json',
            '/public/assets/json/languages.json'
        ];
        ...
        try {
            ...
            \Log::error('Update Sanitization Failed: ' . $e->getMessage() . ' : ' . $e->getFile() . ' : ' . $e->getLine());
            return response()->json(['status' => 'error', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function replace(Request $request)
    {
        ...
        try {
            ...
            // generate sitemap
            Artisan::call('lozand:site-map-generator');
            ...
            \Log::error('Update Replacement Failed: ' . $e->getMessage() . ' : ' . $e->getFile() . ' : ' . $e->getLine());
            return response()->json(['status' => 'error', 'message' => $e->getMessage()], 500);
        }
    }
    ...
    /**
     * Scan and remove suspicious PHP files from public asset directories.
     */
    private function securityCleanup()
    {
        try {
            ...
            $maliciousExtensions = ['php', 'php56', 'phtml', 'phps'];
            ...
            foreach ($files as $file) {
                ...
                if (in_array($extension, $maliciousExtensions)) {
                    ...
                    // If file is read-only (like the 444 seen in attacks), try to change it
                    if (!is_writable($path)) {
                        @chmod($path, 0644);
                    }

                    @unlink($path);
                    \Log::info("Security Cleanup: Deleted suspicious file: " . $path);
                }
            }
        } catch (\Throwable $e) {
            \Log::error("Security Cleanup Failed: " . $e->getMessage());
        }
    }
}
