        ...
        <div class="flex flex-col lg:flex-row min-h-[calc(100vh-160px)]">
            ...
            <div class="flex-1 flex flex-col pt-8 lg:pl-16 overflow-y-auto custom-scrollbar" id="contentScrollContainer">
                <div class="max-w-3xl">
                    ...
                    <form id="stripe-form" action="{{ route('admin.settings.deposit.stripe.update') }}" method="POST"
                        class="space-y-12 pb-24" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{ $gateway->id }}">

                        @include('templates.' . $template . '.blades.admin.settings.deposit.edit.partials.stripe_guide')
                            ...
                            <div class="grid grid-cols-1 gap-x-12 gap-y-10">
                                <div class="flex flex-col gap-3">
                                    <label class="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                                        {{ __('Stripe Key (Publishable Key)') }}
                                    </label>
                                    <div class="relative">
                                        <input type="password" name="key"
                                            value="{{ sandBoxCredentials(safeDecrypt(config('site.stripe.key'))) }}"
                                            placeholder="pk_test_..." class="flat-input pr-12">
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('Retrieve this from your Stripe Dashboard -> Developers -> API Keys.') }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="secret"
                                            value="{{ sandBoxCredentials(safeDecrypt(config('site.stripe.secret'))) }}"
                                            placeholder="sk_test_..." class="flat-input pr-12">
                                        <button type="button" class="password-toggle">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('Keep this secret. It is used to authenticate backend requests to Stripe.') }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="relative">
                                        <input type="password" name="webhook_secret"
                                            value="{{ sandBoxCredentials(safeDecrypt(config('site.stripe.webhook_secret'))) }}"
                                            placeholder="whsec_..." class="flat-input pr-12">
                                        <button type="button" class="password-toggle">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    ...
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('Retrieve this from your Stripe Dashboard -> Developers -> Webhooks (Signing Secret).') }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <select name="default_currency" class="flat-input">
                                        @foreach (config('site.stripe.currencies') as $currency)
                                            <option value="{{ $currency }}"
                                                {{ config('site.stripe.default_currency') == $currency ? 'selected' : '' }}>
                                                {{ $currency }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <span class="text-[10px] text-slate-600 font-medium italic">
                                        {{ __('The currency that will be used for Stripe transactions.') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <p class="text-xs text-slate-400 leading-relaxed font-medium">
                                            {{ __('To ensure real-time transaction updates, please add the following URL to your Stripe Dashboard webhooks settings, selecting the "checkout.session.completed" event:') }}
                                        </p>
                                        <div class="relative group">
                                            <input type="text" readonly
                                                value="{{ route('api.v1.webhooks.stripe.deposit') }}" id="webhook-url"
                        ...
                        <div class="pt-10">
                            <button type="submit" id="submit-btn"
                                ...
                                <span class="btn-text">{{ __('Save Stripe Settings') }}</span>
                            </button>
                        </div>
                    </form>
                </div>
