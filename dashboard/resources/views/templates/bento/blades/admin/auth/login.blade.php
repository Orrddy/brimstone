...
@section('content')
    <div class="space-y-6">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">{{ __('Secure Admin Sign In') }}</h2>
            <p class="text-slate-400 text-sm mt-1">{{ __('Authorised personnel only. All access is monitored and logged.') }}</p>
        </div>
        ...
        <div id="email-login-container" @if (!$show_email_only) style="display: none;" @endif class="space-y-6">
            @if (!$show_email_only)
                <button type="button" onclick="toggleSelection()"
                    class="text-[#c9a84c] hover:text-[#d4af37] text-sm flex items-center gap-2 transition-colors cursor-pointer">
            ...
            <form action="{{ route('admin.login.validate') }}" method="POST" class="ajax-form space-y-5"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="login" id="login"
                        class="w-full bg-slate-800/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-[#c9a84c] focus:ring-1 focus:ring-[#c9a84c] transition-all font-sans text-base"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <input id="remember" name="remember" type="checkbox"
                            class="h-4 w-4 bg-slate-800 border-white/10 rounded text-[#c9a84c] focus:ring-[#c9a84c] focus:ring-offset-0 transition-colors">
                    ...
                    <div>
                        <a href="{{ route('admin.forgot-password') }}"
                            class="text-sm text-[#c9a84c] hover:text-[#d4af37] transition-colors font-medium">
                            {{ __('Reset password') }}
                        </a>
                    </div>
                </div>
                ...
                {{-- Submit Button --}}
                <button type="submit"
                    class="w-full bg-gradient-to-r from-[#0a1628] to-[#1a2744] hover:from-[#1a2744] hover:to-[#0a1628] border border-[#c9a84c]/30 hover:border-[#c9a84c]/60 text-white font-bold py-3.5 px-4 rounded-xl shadow-lg shadow-[#0a1628]/40 transition-all duration-300 transform hover:-translate-y-0.5 relative overflow-hidden group cursor-pointer">
                    <span class="relative z-10">{{ __('Secure Admin Sign In') }}</span>
